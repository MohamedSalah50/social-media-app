...
class AuthenticationService {
    ...
    sendForgotPasword = async (req, res) => {
        ...
        const user = await this.userModel.findOne({ filter: { email, provider: user_model_1.ProviderEnum.System, confirmedAt: { $exists: true } } });
        if (!user) {
        ...
        const otp = (0, otp_1.generateOtp)();
        const result = await this.userModel.updateOne({ filter: { email }, update: { resetPasswordOtp: await (0, hash_security_1.generateHash)(String(otp)) } });
